      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex-1">
          <button
            ...
            disabled={isLoading}
            className="px-3 py-1.5 rounded-lg bg-neutral-800 hover:bg-neutral-700 border border-neutral-700 text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1.5 text-xs"
          >
          {isLoading ? (
            <>
      ...
      {result && (
        ...
        >
          <div className="flex items-start gap-2">
            {result.success ? (
              <LuCheck className="w-4 h-4 text-green-400 flex-shrink-0 mt-0.5" />
            ) : (
              <LuX className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
            )}
            <div className="flex-1">
              <h3
              ...
              {result.error && (
                <p className="text-xs text-red-200 mb-1.5">
                  오류: {result.error}
                </p>
              )}
              {result.data && (
              ...
              {result.details && (
                <details className="mt-1.5">
                  ...
                  <pre className="mt-1.5 text-xs bg-neutral-900 p-1.5 rounded overflow-auto">
                    {result.details}
                  </pre>
                </details>
              )}
